...
function   [funRHS] = asyMemLocalSA_RHS_OGeqn(phiFeed,localCompFlux,fugFeed,params)
...
%fsolve residual functions
    funRHS(1:n) = localCompFlux(1:n)./Vs(1:n)/sum(localCompFlux./Vs(1:n))-ypFinal(1:n);
    
%     funRHS(1:n-1) = sum(ypFinal.*Vs(1:n))*localCompFlux(1:n-1)./Vs(1:n-1)/sum(localCompFlux)-ypFinal(1:n-1);
%     funRHS(n) = 1-sum(sum(ypFinal.*Vs(1:n))*localCompFlux(1:n)./Vs(1:n)/sum(localCompFlux));
%------------------------------------------------------------------------------------------------------------------------------------% 
