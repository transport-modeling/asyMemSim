...
function [B,invGam] = matrixEvalGammaB_FH_LM(stateVar,params)
...
%     end
    ...
    oneMinus = diag(ones(n,1)-phiFH(1:n));
    eyeBones = [eye(n);-ones(1,n)];
    phisMatr = phiFH(1:n+1).*ones(n+1,n);
    fugMatr = stateVar(n+2:end).*ones(n);
    dhdf = diag(Ch.*bs)*(fugMatr*diag(bs./((1+sum(bs.*fs))^2))-diag(1./(1+sum(bs.*fs)))*eye(n)).*stateVar(n+1);
    dgdphiFH = (diag(1./phiFH(1:n))-eye(n)-FH_VFV*(eyeBones+diag(-chis(:,n+1))*phisMatr+diag([upTriChis*phiFH(1:n+1);0])*eyeBones)...
        -diag(diag((FH_VLV+upTriChis)*phisMatr))+oneMinus*(FH_VLV+upTriChis)*eyeBones);
    invGam = -(eye(n)+ones(n).*Ch.*bs.*fs/(1+sum(bs.*fs)))\...
