...
function   [funRHS] = asyMemLocalMSA_RHS(phiFeed,localCompFlux,fugFeed,params,N)
          ...
          % ode15s route
            ...
            if i == 1
                ...
                [z,stateVars] = ode15s(funIVP,zspan,...
                    localCompFlux(n+2+(i-2)*(n+1):(i-1)*(n+1)+n+1),opt); 
            end
          ...
          % DAE ode15i route  
            ...
            opt = odeset('InitialSlope', D0,'RelTol',1e-6,'AbsTol',1e-8);
             if i == 1
                ...
                [W0_new,D0_new] = decic(funIVP,0,localCompFlux(n+2+(i-2)*(2*n+1):(i-1)*(2*n+1)+n+1),...
                    [ones(n+1,1);zeros(n,1)],D0,zeros(1,n+1+n),opt); %FH-DSM
             end
...
%fsolve residual functions
    ...
    if N > 1 && params.memPhaseModel == 1
        funRHS(n+2:(N-1)*(n+1)+n+1) = localCompFlux(n+2:(N-1)*(n+1)+n+1).' - shootingPoints.';
    elseif N > 1 && (params.memPhaseModel == 2 || params.memPhaseModel == 3)
        funRHS(n+2:(N-1)*(2*n+1)+n+1) = localCompFlux(n+2:(N-1)*(2*n+1)+n+1) - shootingPoints;
    end
