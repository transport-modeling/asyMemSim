...
%declare differential variables
    ...
    C_n2 = W(2); %mol/L
    Q = W(3);    %L/hr
    %dQ = W(4); %(only when using NS derived pressure drop)
    P = W(4);  %Pa
...
%volumetric_flow
    ...
    %%[dQ]/[dz] for Hagen-Poisselle deltaP along membrane 
    dQ = -2*pi()*r_i*(J_n2+J_o2)/(C_T)+(Q^2/(101325*3600*1000*R*T*C_T*pi()*r_i^4))*mu*8;   
    %%[dQ]/[dz] accounting for pressure drop ([dC_T]/[dz]=(1/R*T)*d[P]/d[z])    
    %dQ_1 = dQ;
    %dQ_2 = (-C_T*R*T*pi()*r_i^2*101325*1000*3600/(mu*Q)+rho*Q/(mu*pi()*r_i^2*1000*3600))*dQ_1-(2*pi()*r_i*(J_n2+J_o2)*R*T*pi()*r_i^2/(mu*Q))*101325*1000*3600;
...
%NS derived delta P (to test vs. Mass Balance deriverd del P for sanity check)
    %dP = mu/(pi()*r_i^2)*dQ_2*(1/(101325*1000*3600))-rho*Q*dQ_1/(pi()*r_i^2)^2*(1/(101325*1000*1000*3600*3600));
%Hagen-Posielle Pressure Drop
    dP=-8*Q*mu/(pi()*r_i^4*101325*3600*1000);
%differential vector
    %G=[dC_o2;dC_n2;dQ_1;dQ_2;dP]; %for use with NS derived pressure drop
    G=[dC_o2;dC_n2;dQ;dP]; 
